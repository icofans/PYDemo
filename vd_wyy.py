...
class Spider_Model:
    ...
    def user_playlist(self, uid, pageNum):
        action = 'http://changba.com/member/personcenter/loadmore.php?ver=1&type=0&curuserid=-1&pageNum=' + str(
            pageNum) + '&userid=' + uid
    ...
    def Start(self):
        ...
        while i < len(List):
            ...
            while j < self.threadnum and i + j < len(List):
                time.sleep(1)
                downloadThread = DownloadThread(List[i + j][0], List[i + j][1], List[i + j][2], List[i + j][3],
                                                self.uid, j)
            ...
            if (self.ismv.find('none') != -1):
                ...
                if (myItems != None):
                    ...
                    try:
                        ...
                        with open(savePath + "/" + mp3renm + ".mp3", 'w+b') as code:
                            code.write(data)
                        print
                        self.threadnm, u'歌曲', mp3renm, u"下载成功"
                        code.close()
                    except IOError, e:
                        print
                        self.threadnm, u'歌曲', mp3renm, u"下载失败"
                        print("open exception: %s: %s\n" % (e, e.strerror))
                else:
                    print
                    self.threadnm, u'歌曲未找到'
                ...
                try:
                    ...
                    with open(savePath + "/" + mp3renm + ".mp4", 'w+b') as code:
                        code.write(data)
                    print
                    self.threadnm, u'视频', mp3renm, u"下载成功"
                    code.close()
                except IOError, e:
                    print
                    self.threadnm, u'视频', mp3renm, u"下载失败"
                    print("open exception: %s: %s\n" % (e, IOError))
        except Exception, e:
            print
            u'下载失败,', self.threadnm, '退出'
            print("exception: %s: %s\n" % (e, Exception))
            return None
...
if __name__ == '__main__':
    url = "http://kg.qq.com/node/personal?uid=639b9c85212e308b"
    test()
